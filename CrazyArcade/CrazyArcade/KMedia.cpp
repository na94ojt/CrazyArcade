...
void KMedia::Open(LPWSTR filename)

{

    mciOpenParms.lpstrElementName = filename;                           //화일 설정

    mciOpenParms.lpstrDeviceType = L"MPEGVideo";                     //디바이스 를 waveaudio로
...
void KMedia::PlayWave(LPWSTR filename, int ch)
{
	if( 0 == ch )
	{
		if( -1 != m_h1)
	...
	MCI_PLAY_PARMS b;
	DWORD r;
	a.lpstrDeviceType = L"WaveAudio";
	a.lpstrElementName = (LPWSTR)filename;
	// 사운드 카드를 열어 핸들을 확보
